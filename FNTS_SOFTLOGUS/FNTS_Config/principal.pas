...
uses
  ...
  Spin, Printers, GravaIni, sSkinManager;
...
type
  TfrmPrincipal = class(TForm)
    ...
    advrflctnmg2: TAdvReflectionImage;
    advrflctnmg3: TAdvReflectionImage;
...
begin
  ...
  repeat
    ...
    if smsg <> 'OK' then
    begin
      if application.messagebox(PWideChar('Erro ao tentar abrir a porta - ' + sECF_porta + #13 + 'Deseja tentar outra vez?'), 'ECF',
  ...
  begin
    ...
    repeat
      ...
      if smsg <> 'OK' then
      begin
        if application.messagebox(PWideChar('ECF desligado ou não conectado!' + #13 + 'Mensagem: ' + smsg + #13 +
          'Deseja tentar outra vez?'), 'ECF', mb_yesno + mb_iconerror) = idNo then
        begin
          break;
        end
        else
...
begin
  ...
  // s7 - arquivo de configuracao

 {
  AssignFile(TXT, ExtractFilePath(application.ExeName) + 'Cfg\cfg.ini');
  rewrite(TXT);
  ...
  writeln(TXT, 'impnaof ' + ComboImpressora.Text);
  writeln(TXT, 'gavnaof ' + ComboPortaGaveta.Text);
  writeln(TXT, 'qtdlfin ' + EditQtdLinhas.Text);
  closefile(TXT);
}
...
begin
  ...
  rewrite(F);
  writeln(F, 'Softlogus - IMPRESSAO - 1');
  writeln(F, 'Softlogus - IMPRESSAO - 2');
  writeln(F, 'Softlogus - IMPRESSAO - 3');
  writeln(F, 'Softlogus - IMPRESSAO - 4');
