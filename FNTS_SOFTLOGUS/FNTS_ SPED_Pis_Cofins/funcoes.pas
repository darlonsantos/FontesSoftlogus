...
//  result := true;
  ...
  if sECF_Serial <> Trim(TrimLeft(TrimRight( Cript('D',Arquivo_ini.ReadString('B9A7BF57','D875EF1310639C',''))))) then
  begin
...
begin
  ...
  if (cst = '041') or  (cst = '050') or  (cst = '051') or  (cst = '090') then
  begin
    if COD_ECF = EPSON then
      Result := 'N'
    else
