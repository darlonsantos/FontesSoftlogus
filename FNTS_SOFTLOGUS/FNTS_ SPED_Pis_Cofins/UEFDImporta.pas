...
uses Forms, strutils, classes, sysutils, Vcl.StdCtrls, Vcl.Graphics, UFuncoes, db, dialogs;
...
begin
  ...
  begin
    ...
    FPrincipal.Gauge1.MaxValue := efd.Lines.Count;
    FPrincipal.Gauge1.Progress := 0;
    FPrincipal.Gauge1.ForeColor := clLime;
    for I := 0 to efd.Lines.Count - 1 do
    begin
      { ecf's }
      ...
      begin
        ...
        begin
          ...
          qrC400_ECFDT_INI.Value := StrToDate(DateToStr(DT_INI));
          qrC400_ECFDT_FIM.Value := StrToDate(DateToStr(DT_FIM));
          qrC400_ECF.Post;
      ...
      FPrincipal.Gauge1.Progress := FPrincipal.Gauge1.Progress + 1;
      Application.ProcessMessages;
    end;

    FPrincipal.Gauge1.MaxValue := efd.Lines.Count * qrC400_ECF.RecordCount;
    FPrincipal.Gauge1.Progress := 0;
    FPrincipal.Gauge1.ForeColor := clGreen;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (StrToDate(DateToStr(DataEFD(LinhaEFD(efd.Lines[I], '|', 3)))) >= DT_INI) and
            (StrToDate(DateToStr(DataEFD(LinhaEFD(efd.Lines[I], '|', 3)))) <= DT_FIM) then
          begin
            ...
            qrC405_REDUCOESDT_DOC.Value := StrToDate(DateToStr(DataEFD(LinhaEFD(efd.Lines[I], '|', 3))));
            ...
            FPrincipal.Msg('  C405 - Redução Z  - ' + qrC405_REDUCOESDT_DOC.AsString + ' - ' + qrC405_REDUCOESNUM_COO_FIN.AsString,
              10, clBlack);
          end;
          Application.ProcessMessages;
        end;
        ...
        begin
          if (StrToDate(DateToStr(DataEFD(LinhaEFD(efd.Lines[I], '|', 6)))) >= DT_INI) and
            (StrToDate(DateToStr(DataEFD(LinhaEFD(efd.Lines[I], '|', 6)))) <= DT_FIM) then
          begin
            ...
            qrC460_CUPONSDT_DOC.Value := StrToDate(DateToStr(DataEFD(LinhaEFD(efd.Lines[I], '|', 6))));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FPrincipal.MemoEFD.Lines.LoadFromFile(ListaEFDECF.Items[0]);
...
begin
  ...
  begin
    ...
    AddItemListView('Importação Registros ECF', icProcessa);
    Msg('', 08, clBlue);
    Msg('0200 - IDENTIFICAÇÃO DOS PRODUTOS', 10, clBlue);
    Msg('', 08, clBlue);

    { produtos }
    AddItemListView('Importando produtos', icProcessa);
    ...
    Msg('', 08, clBlue);
    Msg('C400 - REGISTROS ECF', 10, clBlue);
    Msg('', 08, clBlue);

    { importa registros ECF completo }
    AddItemListView('Importando movimentações', icProcessa);
