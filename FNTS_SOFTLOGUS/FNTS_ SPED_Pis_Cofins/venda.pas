...
uses
  ...
  AdvMenus,principal, FileCtrl, FlCtrlEx, clipbrd, ImgList,
  dxGDIPlusClasses, ActnList, XPStyleActnCtrls, ActnMan, StrUtils;
...
begin
  ...
  if bVenda then
    gravaINI(ExtractFilePath(Application.ExeName) + 'cfg\cfg.ini','PDV','Aberto','SIM' )
   else
    gravaINI(ExtractFilePath(Application.ExeName) + 'cfg\cfg.ini','PDV','Aberto','NAO' );
...
//  frmModulo.Conexao_Servidor.Close;
  try
    ...
    begin
      ...
      sNumero_Cupom := Zerar( FloatToStr(  Max('cupom','numero','') + 1 ), 5  );
      sNumero_contador_cupom :=  Zerar( FloatToStr(  Max('cupom','CCF','') + 1 ),  5  );
  ...
  gravaINI(ExtractFilePath(Application.ExeName) + 'cfg\cfg.ini','PDV','NumCupom',sNumero_Cupom);
...
begin
  ...
  begin
    Imprime_display_teclado('Mandrake Automação', 'Siscom 8.5');
...
begin
 ...
 {$ENDIF}
  ...
  iImpressora := StrToInt( frmPrincipal.LerINi( '.\CFG\cfg.ini','Impressora','TIPO','0' ) );
 ...
 begin
  ...
  begin
    // abrir a comunicacao
    OpenTec44;
    Imprime_display_teclado('SolivanSoft Sistemas :: BR Sistemas 9.0','Terminal iniciado com sucesso!');
  end;
...
begin
  ...
  END;
    ...
    begin
      ...
      // Abrir a gaveta...
      if frmPrincipal.TipoImpressora = Fiscal then
        sMsg := cECF_Abre_Gaveta(iECF_Modelo); // Fiscal

      if frmPrincipal.TipoImpressora = NaoFiscal then
      begin                                     // Nao Fiscal
        sMsg := ok;
        AcionaGavetaNaoFiscal(sPortaNaoFiscal);        
      end;
...
begin
  ...
  begin
    ...
    if Executa_Login('MESTRESISCOM8',ed_senha.text,'') then
    begin
      bSair_campo := true;
...
begin
  ...
  // verificar se existem cupons abertos e fazer o tratamento //**
     if (frmPrincipal.TipoImpressora = Fiscal) then
      ...
      sCupomAbertoECF := lerINI(ExtractFilePath(Application.ExeName) + 'cfg\cfg.ini','PDV','Aberto','NAO');
...
begin
  ...
  frmPrincipal.GravaIni('.\CFG\cfg.ini','Impressora','TIPO',IntToStr(iImpressora));
  

  if iImpressora >= 3 then
    iImpressora := -1;
