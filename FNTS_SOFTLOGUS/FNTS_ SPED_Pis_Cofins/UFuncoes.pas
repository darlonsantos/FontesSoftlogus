...
begin
  try
    Dm.qrCMD.close;
    Dm.qrCMD.SQL.Clear;
    Dm.qrCMD.SQL.Add('Select max(' + sCampo + ') as ma from ' + sTabela);
    if length(sCondicao) > 0 then
      Dm.qrCMD.SQL.Add(sCondicao);

    Dm.qrCMD.Open;

    result := zerarcodigo(IntToStr(isInteger(Dm.qrCMD.FieldByName('ma').AsString) + 1), 6);
