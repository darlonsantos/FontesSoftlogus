...
type
  Tfrmnotafiscal_menu = class(TForm)
    ...
    qrTrib: TZQuery;
    qryApoioCCE: TZQuery;
    qrnotaOBS_NFE: TWideMemoField;
    qrnotaTIPO_FINALIDADE: TWideStringField;
    ACBrNFe1: TACBrNFe;
...
{$ENDIF}
    { TODO -oMizael -cNFe : configurações corretas do componente acbr }
...
var
  ...
  ValorTotalAgregacaoItem: Double;
  NumeroItens: integer;
  ValorPorItem: Double;
  ValorUltimo: Double;
begin
  ...
  begin
    ...
    then
      Ide.finNFe := fnAjuste
    else if frmnotafiscal_menu.qrnota.fieldbyname('TIPO_FINALIDADE').asstring = '4 - DEVOLUÇÃO/RETORNO'
    then
    ...
    qryapoio.ParamByName('NUMNOTA').asstring :=
      qrnota.fieldbyname('numero').asstring;
    qryapoio.open;

    if qryapoio.RecordCount > 0 then
    begin
    ...
    if qrNFE_Cliente.fieldbyname('tipo').AsInteger = 1 then // fisica
    begin
      Dest.IE := ''; // tem que conter a literal “ISENTO”
      Dest.indIEDest := inNaoContribuinte;

    end
    else if qrNFE_Cliente.fieldbyname('tipo').AsInteger = 2 then // juridica
    begin

      if qrNFE_Cliente.fieldbyname('rg').asstring = 'ISENTO' then
      begin
        Dest.IE := 'ISENTO'; // tem que conter a literal “ISENTO”
    ...
    qrnota_item.Last;
    qrnota_item.first;
    ValorTotalAgregacaoItem := qrnota.fieldbyname('OUTRAS_DESPESAS').asfloat;
    NumeroItens := qrnota_item.RecordCount;
    ValorPorItem := ValorTotalAgregacaoItem / NumeroItens;
    ValorPorItem := strtofloat(formatfloat('0.00', ValorPorItem));
    ...
    begin
      ...
      begin
        ...
        with Det.add do
        begin
          // aqui comecao o while de produtos
          ...
          Prod.vSeg := qrnota.fieldbyname('seguro').AsCurrency;
          // mizael 07/04/2015
          Prod.vOutro := strtofloat((formatfloat('0.00', ValorPorItem)));
          ValorUltimo := ((ValorPorItem * (NumeroItens - 1)) -
            (ValorTotalAgregacaoItem));
          ValorUltimo := strtofloat(StringReplace(vartostr(ValorUltimo), '-',
            '', [rfReplaceAll, rfIgnoreCase]));
          Prod.vOutro := strtofloat((formatfloat('0.00', ValorUltimo)));
          // mizael 29/03/2015
            ...
            case iCRT of
                  ...
                  case ICMS.CSOSN of
                    ...
                    csosn101:
                      begin
                        ...
                        ICMS.vCredICMSSN :=
                          (qrnota_item.fieldbyname('valor_icms').asfloat *
                          qrnota_item.fieldbyname('ICMS').asfloat) / 100;
              ...
              3:
                begin
                  ...
                  // 013 - Alíquota do IPI
                  IPI.vIPI := RoundTo(qrnota_item.fieldbyname('valor_ipi')
                    .asfloat, -2);
                  // O14 - Valor do IPI
    ...
    Transp.Transporta.xEnder := Trim(qrtransportador2.fieldbyname('endereco')
      .asstring) + ', ' + Trim(qrtransportador2.fieldbyname('numero').asstring);
    ...
    InfAdic.infCpl := qrnota.fieldbyname('obs_nfe').Value;
...
begin
  ACBrNFeDANFEFR1.FastFile := 'C:\Softlogus\Server\nfe\danfe\DANFeRetrato.fr3';
...
begin
  try
    ...
    begin
      ...
      pdf := 'C:\Softlogus\SERVER\PDF\' + StringReplace(ExtractFileName(sXML),
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not(InputQuery('NFe Cancelamento por Evento',
          'Informe a justificativa', vAux)) then
          exit;
      ...
      begin
        ...
        BEGIN
          ...
          if qrnota_item.fieldbyname('codgrade').asstring <> '' then
          begin
            IF frmnotafiscal_menu.qrnota.fieldbyname('BAIXA_ESTOQUE')
              .asstring = 'S' THEN
            BEGIN
              frmmodulo.qrgrade_produto.close;
              frmmodulo.qrgrade_produto.sql.clear;
    ...
    begin
      Application.messagebox('Esta nosta fiscal já está CANCELADA!', 'Erro',
        mb_ok + mb_iconerror);
    end;
    // end;
  end
...
begin
  ...
  Ano := formatdatetime('yyyy', StrToDate(Ano));
  serie := '1';
  // qrempresa.fieldbyname('CONHECIMENTO').asstring;
  modelo := '55';
  NumeroInicial := qrnota.fieldbyname('NUMERO').asstring;
  NumeroFinal := qrnota.fieldbyname('NUMERO').asstring;
  ...
  try
    ...
    begin
      ...
      qrnota.edit;
      qrnota.fieldbyname('nfe_situacao').AsInteger := 9;
      qrnota.post;

      Application.messagebox('Inutilização de número homologado com sucesso!!!',
        'Aviso', mb_ok);
    end;

  except
    on E: Exception do
      Application.messagebox(PWideChar('Motivo : ' + E.message), 'Erro',
        mb_ok + mb_iconerror);
  end;
