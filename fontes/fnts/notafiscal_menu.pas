...
type
  Tfrmnotafiscal_menu = class(TForm)
    ...
    zqryComplementarNUMNOTA: TWideStringField;
...
begin
  ...
  begin
   ...
   if Copy(qrnota.fieldbyname('NATUREZA').AsString, 1, 5) = 'VENDA' then
      ...
      if Copy(qrnota.fieldbyname('NATUREZA').AsString, 1, 5) = 'COMPL' then
          begin
                 Ide.finNFe := fnComplementar;
              //   Ide.NFref.Add.RefNfe := qrnota.fieldbyname('INF2').AsString;
          end
   else
      if Copy(qrnota.fieldbyname('NATUREZA').AsString, 1, 5) = 'AJUST' then
            begin
                   Ide.finNFe := fnAjuste;
            end
   else
       if Copy(qrnota.fieldbyname('NATUREZA').AsString, 1, 5) = 'DEVOL' then
          begin
                 Ide.finNFe := fnDevolucao;
                 Ide.NFref.Add.RefNfe := zqryComplementar.fieldbyname('chave').AsString;
          end;
    ...
    //Dest.IE := SN(qrNFE_Cliente.fieldbyname('rg').asstring);
      if (qrNFE_Cliente.fieldbyname('tipo').AsInteger = 1) then
           ...
           End;
            ...
            ((qrNFE_Cliente.fieldbyname('rg').asstring = 'ISENTO')
            or (qrNFE_Cliente.fieldbyname('rg').asstring = ''))
            then
           Begin
          ...
          if (qrNFE_Cliente.fieldbyname('tipo').AsInteger = 2) and (qrNFE_Cliente.fieldbyname('rg').asstring <> '') and (qrNFE_Cliente.fieldbyname('rg').asstring <> 'ISENTO') Then
           Begin
                Dest.IE := SN(qrNFE_Cliente.fieldbyname('rg').asstring);
                 ...
                 begin
                       with ICMSUFDest do
                              begin
                                ...
                                pICMSInterPart := frmnotafiscal_menu.qrnota.fieldbyname('PICMSINTERPART').asfloat;
      ...
      if xTotalImposto > 0 then
        Total.ICMSTot.vTotTrib := xTotalImposto;
          ...
          // partilha do icms e fundo de probreza
           if (query2.fieldbyname('UF').asstring <> qrNFE_Cliente.fieldbyname('uf').asstring) and (qrNFE_Cliente.fieldbyname('tipo').AsInteger = 1) then
                 begin
                       Total.ICMSTot.vFCPUFDest   := frmnotafiscal_menu.qrnota.fieldbyname('VFCPUFDEST').asfloat;
                       Total.ICMSTot.vICMSUFDest  := frmnotafiscal_menu.qrnota.fieldbyname('VICMSUFDEST').asfloat;
                        Total.ICMSTot.vICMSUFRemet := frmnotafiscal_menu.qrnota.fieldbyname('VICMSUFREMET').asfloat;
                 end
                 else
                 begin
...
begin
  ...
  if sXML = '' then
    ...
    try
      CC.Add('cliente@cliente.com'); //especifique um email vÃ¡lido
      CC.Add('cliente@cliente.com');    //especifique um email vÃ¡lido
