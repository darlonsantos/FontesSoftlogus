...
uses
  ...
  IdTCPConnection, IdTCPClient, IdMessageClient, IdSMTP,IdAttachment,IdAuthentication, IdAttachmentFile;
...
begin
  ...
  AdicionaUsuario.FLines.Add
    ('<html> <head> <title>Inclusão de usuário</title> <style type="text/css"> <!-- body { 	margin-left: 0px; '
    + #13#10 +
    'margin-top: 0px; 	margin-right: 0px; 	margin-bottom: 0px; } --> </style></head>'
    + #13#10 + '<body> <p>Atencao: <br>Seu cadastro no sistema iCloud foi realizado com sucesso :</p>' + #13#10 +
  ...
  AlteraUsuario.FLines.Add
    ('<html> <head> <title>Alteração de usuário</title> <style type="text/css"> <!-- body { 	margin-left: 0px; '
    + #13#10 +
    'margin-top: 0px; 	margin-right: 0px; 	margin-bottom: 0px; } --> </style></head>'
    + #13#10 +
    '<body> <p>Atenção: <br>Por motivos operacionais seus dados ou acessos no sistema iCloud foram alterados</p>'
    + #13#10 +
    '<table width="100%" border="0" cellspacing="2" cellpadding="0"> ' + #13#10
    + '<tr> ' + #13#10 +
    ' <td width="10%" align="right"><strong>Nome ..:&nbsp;</strong></td>' +
  ...
  SenhaForcada.FLines.Add
    ...
    'margin-top: 0px; 	margin-right: 0px; 	margin-bottom: 0px; } --> </style></head>'
    + #13#10 +
    '<body> <p>Atencao: <br> Voce ou um administrador forcou a troca de sua senha do sistema iCloud, sua senha foi alterada para a senha abaixo:</p>'
    + #13#10 +
    '<table width="100%" border="0" cellspacing="2" cellpadding="0"> ' + #13#10
    + '<tr> ' + #13#10 +
    ' <td width="10%" align="right"><strong>Nome ..:&nbsp;</strong></td>' +
...
procedure TMailUserControl.EnviaEmailAdicionaUsuario(Nome, Login, Senha, Email,
  Perfil: String; Key: Word);
begin
  //Senha := TSenhaForm(FormSenha).edtSenha.Text;//TrataSenha(Senha, Key, False, 0);
...
begin
  ...
  EmailCorpo := TIdMessage.Create(Self);
  SMTP := TIdSMTP.Create(Self);
  // prepara email
  EmailCorpo.ContentType := 'text/html';
  EmailCorpo.Body.Add(ParseMailMSG(Nome, Login, USenha, Email, Perfil,
    UCMSG.Mensagem.Text));
  EmailCorpo.From.Name := TUserControl(fUsercontrol).CurrentUser.UserName;
  // remetente
  EmailCorpo.From.Address := EmailRemetente;
  EmailCorpo.Recipients.EMailAddresses := Email;
  EmailCorpo.Priority := mpHighest;
  EmailCorpo.Subject := UCMSG.Titulo; // assunto

  SMTP.Host := ServidorSMTP;
  SMTP.AuthType := satDefault;
  SMTP.Port := FPorta;
  SMTP.UserName := EmailRemetente; // seu login
  SMTP.Password := '159357te'; // sua senha
  SMTP.Connect;
  ...
  try
    try
      ...
      UCEMailForm.lbStatus.Caption := 'Email enviado com sucesso';
      UCEMailForm.Update;
      Result := True;
    except
      on e: Exception do
