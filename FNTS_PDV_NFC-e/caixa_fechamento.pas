...
begin
  {Anexo VI -  Arquivo eletronico dos registros Efetuados pelo ECF
   ...
   try
    ...
    ibcqryArquivo.close;
    ibcqryArquivo.sql.clear;
    ibcqryArquivo.sql.add('Select * from reducaoZ');
    ibcqryArquivo.sql.add('where data_movimento = :datai');
    ibcqryArquivo.sql.add('and ecf = '''+sECF_Serial+'''');
    ibcqryArquivo.sql.add('order by data_movimento');
    ibcqryArquivo.parambyname('datai').asdatetime := data;
    ibcqryArquivo.open;
    ibcqryArquivo.first;

    // gerar um arquivo R02 para cada reduzao Z emitida
    while not ibcqryArquivo.eof do
    begin
      //01 - Tipo
      write(txt,'R02');
      ...
      //06 CRZ - 6
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('CRZ').asstring,6,'0',taDireita));
      //07 COO - 6
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('COO').asstring,6,'0',taDireita));
      //08 CRO - 6
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('CRO').asstring,6,'0',taDireita));
      //09 Data do Movimento - 8 - AAAAMMDD
      write(txt,troca_data(ibcqryArquivo.fieldbyname('data_movimento').asstring));
      //10 Data de Emissao - 8 - AAAAMMDD
      write(txt,troca_data(ibcqryArquivo.fieldbyname('data_emissao').asstring));
      //11 Hora de Emissao - 6 - HHMMSS
      write(txt,texto_justifica(somenteNumero(ibcqryArquivo.fieldbyname('hora_emissao').asstring),6,' ',tadireita));
      //12 Venda bruta diária - 14 - 00000000001011 -> R$ 10,11
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('venda_bruta').asfloat)),14,'0',taDireita));
    ...
    ibcqryArquivo.close;
    ibcqryArquivo.sql.clear;
    ibcqryArquivo.sql.add('select reducaoz_total_parcial.*, reducaoz.CRZ, reducaoz.data_movimento');
    ibcqryArquivo.sql.add('from reducaoz_total_parcial, reducaoz');
    ibcqryArquivo.sql.add('where reducaoz_total_parcial.cod_reducaoz = reducaoz.codigo');
    ibcqryArquivo.sql.add('and reducaoz.data_movimento =  :datai');
    ibcqryArquivo.sql.add('and reducaoz.ecf = '''+sECF_Serial+'''');
    ibcqryArquivo.sql.add('order by reducaoz.data_movimento, reducaoz_total_parcial.codigo');
    ibcqryArquivo.parambyname('datai').asdatetime := data;
    ibcqryArquivo.open;
    ibcqryArquivo.first;
    ...
    begin
      ...
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('totalizador').asstring,7,' ',taEsquerda));
      //08 - Valor acumulado - 13
      writeln(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor').asfloat)),13,'0',taDireita));
      ibcqryArquivo.next;
    end;
    ...
    ibcqryArquivo.close;
    ibcqryArquivo.sql.clear;
    ibcqryArquivo.sql.add('select * from cupom');
    ibcqryArquivo.sql.add('where data = :datai');
    ibcqryArquivo.sql.add('and ecf = '''+sECF_Serial+'''');
    ibcqryArquivo.sql.add('order by data, codigo');
    ibcqryArquivo.parambyname('datai').asdatetime := data;
    ibcqryArquivo.open;
    ibcqryArquivo.first;
    ...
    begin
      ...
      write(txt,texto_justifica(sECF_usuario,2,'0',taDireita));
      //06 CCF
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('ccf').asstring,6,'0',taDireita));
      //07 COO
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('numero').asstring,6,'0',taDireita));
      //8 Data de Emissao - 8 - AAAAMMDD
      write(txt,troca_data(ibcqryArquivo.fieldbyname('data').asstring));
      //9 Subttotal do documento - 14
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor_produto').asfloat)),14,'0',taDireita));
      //10 Desconto sobre subtotal - 13
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor_desconto').asfloat)),13,'0',taDireita));
      //11 Indicador do tipo de desconto - V ou P
      write(txt,'V');
      //12 Acrescimo sobre subtotal - 13
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor_acrescimo').asfloat)),13,'0',taDireita));
      //13 Indicador do tipo de acrescimo - V ou P
      write(txt,'V');
      //14 Valor total liquido - 14
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor_TOTAL').asfloat)),14,'0',taDireita));
      //15 Indicador de cancelamento
      if ibcqryArquivo.fieldbyname('cancelado').asinteger = 1 then
        write(txt,'S')
      else
        write(txt,'N');
      ...
      if ibcqryArquivo.fieldbyname('valor_desconto').asfloat > 0 then
        write(txt,'D')
      else
         if ibcqryArquivo.fieldbyname('valor_acrescimo').asfloat > 0 then
           write(txt,'A')
         else
           write(txt,' ');
      //18 Nome do cliente - 40
      Write(txt,texto_justifica(ibcqryArquivo.fieldbyname('nome_consumidor').asstring,40,' ',taEsquerda));
      //18 CPF/CNPJ do cliente - 14
      Writeln(txt,texto_justifica(somenteNumero(ibcqryArquivo.fieldbyname('cpf_consumidor').asstring),14,' ',taEsquerda));
      ibcqryArquivo.next;
    end;
    ...
    ibcqryArquivo.close;
    ibcqryArquivo.sql.clear;
    ibcqryArquivo.sql.add('select cupom_item.*, cupom.numero, cupom.ccf, cupom.data, estoque.nome produto');
    ibcqryArquivo.sql.add('from cupom_item, cupom, estoque');
    ibcqryArquivo.sql.Add('where cupom_item.cod_cupom = cupom.codigo');
    ibcqryArquivo.sql.add('and cupom_item.cod_produto = estoque.codigo');
    ibcqryArquivo.sql.add('and cupom.data = :datai');
    ibcqryArquivo.sql.add('and cupom.ecf = '''+sECF_Serial+'''');
    ibcqryArquivo.sql.add('order by cupom.data, cupom.numero, cupom_item.item');
    ibcqryArquivo.parambyname('datai').asdatetime := data;
    ibcqryArquivo.open;
    ibcqryArquivo.first;
    ...
    begin
      ...
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('ccf').asstring,6,'0',taDireita));
      //08 Numero do Item
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('item').asstring,3,'0',taDireita));
      //09 Codigo do Produto
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('cod_produto').asstring,14,' ',taEsquerda));
      //10 Descricao do produto
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('produto').asstring,100,' ',taEsquerda));
      //11 Quantidade - 7
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.000',ibcqryArquivo.fieldbyname('qtde').asfloat)),7,'0',taDireita));
      //12 Descricao do produto
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('unidade').asstring,3,' ',taEsquerda));
      //13 Valor Unitario - 8
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.000',ibcqryArquivo.fieldbyname('valor_unitario').asfloat)),8,'0',taDireita));
      //14 Desconto - 8
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor_desconto').asfloat)),8,'0',taDireita));
      //15 Acrescimo - 8
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor_acrescimo').asfloat)),8,'0',taDireita));
      //16 Valor Total Liquido - 14
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor_total').asfloat)),14,'0',taDireita));
      //17 Totalizador Parcial (aliquota - conforme tabela);
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('cod_totalizador').asstring,7,' ',taEsquerda));
    ...
    ibcqryArquivo.close;
    ibcqryArquivo.sql.clear;
    ibcqryArquivo.sql.add('select * from nao_fiscal');
    ibcqryArquivo.sql.add('where ecf = '''+sECF_Serial+'''');
    ibcqryArquivo.sql.add('and data = :datai');
    ibcqryArquivo.sql.add('order by data, codigo');
    ibcqryArquivo.parambyname('datai').asdatetime := data;
    ibcqryArquivo.open;
    ibcqryArquivo.first;
    ...
    begin
      ...
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('coo').asstring,6,'0',taDireita));
      //07 gnf
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('gnf').asstring,6,'0',taDireita));
      //08 grg
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('grg').asstring,6,'0',taDireita));
      //09 cdc - 4
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('cdc').asstring,4,'0',taDireita));
      //10 denominacao - 2
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('denominacao').asstring,2,' ',taEsquerda));
      //11 Data de Emissao - 8 - AAAAMMDD
      write(txt,troca_data(ibcqryArquivo.fieldbyname('data').asstring));
      //12 Hora de Emissao - 6 - HHMMSS
      writeln(txt,texto_justifica(somenteNumero(ibcqryArquivo.fieldbyname('hora').asstring),6,' ',tadireita));
    ...
    ibcqryArquivo.close;
    ibcqryArquivo.sql.clear;
    ibcqryArquivo.sql.add('select');
    ibcqryArquivo.sql.add('  COO,');
    ibcqryArquivo.sql.add('  CCF,');
    ibcqryArquivo.sql.add('  GNF,');
    ibcqryArquivo.sql.add('  Forma,');
    ibcqryArquivo.sql.add('  Valor');
    ibcqryArquivo.sql.add('from');
    ibcqryArquivo.sql.add('  (Select');
    ibcqryArquivo.sql.add('     cupom.numero as COO,');
    ibcqryArquivo.sql.add('     cupom.ccf as ccf,');
    ibcqryArquivo.sql.add('     ''000000'' as gnf,');
    ibcqryArquivo.sql.add('     cupom_forma.forma as Forma,');
    ibcqryArquivo.sql.add('     cupom_forma.valor as Valor');
    ibcqryArquivo.sql.add('   from');
    ibcqryArquivo.sql.add('     cupom_forma, cupom');
    ibcqryArquivo.sql.add('   where');
    ibcqryArquivo.sql.add('     cupom_forma.cod_cupom = cupom.codigo and');
    ibcqryArquivo.sql.add('     cupom.data = :datai and cupom.cancelado = 0 and');
    ibcqryArquivo.sql.add('     cupom.ecf = '''+sECF_Serial+'''');
    ibcqryArquivo.sql.add('   union');
    ibcqryArquivo.sql.add('   Select');
    ibcqryArquivo.sql.add('     nao_fiscal.coo as COO,');
    ibcqryArquivo.sql.add('     ''000000'' as ccf,');
    ibcqryArquivo.sql.add('     nao_fiscal.gnf as gnf,');
    ibcqryArquivo.sql.add('     cupom_forma.forma as Forma,');
    ibcqryArquivo.sql.add('     cupom_forma.valor as Valor');
    ibcqryArquivo.sql.add('   from');
    ibcqryArquivo.sql.add('     cupom_forma, nao_fiscal');
    ibcqryArquivo.sql.add('   where');
    ibcqryArquivo.sql.add('     cupom_forma.cod_cupom = nao_fiscal.codigo and');
    ibcqryArquivo.sql.add('     nao_fiscal.data = :datai and');
    ibcqryArquivo.sql.add('     nao_fiscal.ecf = '''+sECF_Serial+''') as TMP');
    ibcqryArquivo.parambyname('datai').asdatetime := data;
    ibcqryArquivo.open;
    ibcqryArquivo.first;
    ...
    begin
      ...
      write(txt,texto_justifica(ibcqryArquivo.fieldbyname('forma').asstring,15,' ',taEsquerda));
      //10 Valor Pago - 13
      write(txt,texto_justifica(somenteNumero(formatfloat('##########0.00',ibcqryArquivo.fieldbyname('valor').asfloat)),13,'0',taDireita));
