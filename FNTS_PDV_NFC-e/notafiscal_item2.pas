...
begin
  ...
  begin
    ...
    qrproduto.sql.clear;
    qrproduto.sql.add
      ('select prod.*, est.* from c000025 prod, c000100 est where prod.codigo = est.codproduto and  codbarra = '''
      + parametro + '''');
    qrproduto.open;
    if qrproduto.RecordCount = 0 then
    begin
      ...
      qrproduto.sql.clear;
      qrproduto.sql.add
        ('select prod.*, est.* from c000025 prod, c000100 est where prod.codigo = est.codproduto and  upper(produto) like '''
        + ansiuppercase(parametro) + '%''');
      qrproduto.open;
      if qrproduto.RecordCount = 0 then
      begin
        qrserial.close;
        qrserial.sql.clear;
        qrserial.sql.add('select * from c000022 where serial = ''' + parametro +
          ''' and situacao = 1 ');
        qrserial.open;
        ...
        begin
          ...
          qrproduto.sql.add
            ('select prod.*, est.* from c000025 prod, c000100 est where prod.codigo = est.codproduto and  codigo = '''
            + qrserial.fieldbyname('codproduto').asstring + '''');
          qrproduto.open;
  ...
  begin
    ...
    begin
      ...
      begin
        qrserial.close;
        qrserial.sql.clear;
        qrserial.sql.add('select * from c000022 where codproduto = ''' +
          qrproduto.fieldbyname('codigo').asstring + ''' and situacao = 1 ');
        qrserial.open;
        if qrserial.RecordCount > 0 then
        begin
          resultado_pesquisa1 := '';
          ...
          begin
            qrserial.close;
            qrserial.sql.clear;
            qrserial.sql.add('select * from c000022 where serial = ''' +
              resultado_pesquisa1 + ''' and situacao = 1 ');
            qrserial.open;
            nr_serial := qrserial.fieldbyname('serial').asstring;
          end;
        end
        ...
        begin
          ...
          begin
            ...
            begin
              resultado_pesquisa1 := '';
              frmnotafiscal_item_grade :=
                tfrmnotafiscal_item_grade.create(self);
              ...
              begin
                qrgrade.close;
                qrgrade.sql.clear;
                qrgrade.sql.add('select * from c000021 where codigo = ''' +
                  resultado_pesquisa1 + '''');
                qrgrade.open;
      ...
      ed_classific.text := qrproduto.fieldbyname
      ...
      then
      begin
        ed_complemento.lines.text := qrproduto.fieldbyname('aplicacao')
          .asstring;
      end
      else
      ...
      begin
        query1.close;
        query1.sql.clear;
        query1.sql.add('select * from c000026 where codproduto = ''' +
          qrproduto.fieldbyname('codigo').asstring + '''');
        query1.open;
  ...
  begin
    application.messagebox('Produto não localizado!', 'Erro',
      mb_ok + mb_iconerror);
    ed_produto.SelectAll;
    result := false;
  end;
...
begin
  ...
  if ed_cst.text = '' then
  begin
    application.messagebox('Favor informar a CST!', 'Erro',
      mb_ok + mb_iconerror);
    ed_cst.setfocus;
    exit;
  end;
  ...
  begin
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      ('CLASSIFICACAO_FISCAL,CST,VALOR_ICMS,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      ('ICMS_REDUZIDO,BASE_CALCULO,MARGEM_AGREGADA,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      ('BASE_SUB,ICMS_SUB,ISENTO,CODLANCAMENTO,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      ('DESCONTO,SUBTOTAL,PESO_BRUTO,PESO_LIQUIDO,Item,codigo,grade,complemento,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      ('pis_base,pis_valor,pis_aliquota,cofins_base,cofins_valor,cofins_aliquota)');
    FrmNotaFiscal.qrnotafiscal_item.sql.add('VALUES');
    FrmNotaFiscal.qrnotafiscal_item.sql.add('(:CODNOTA,:CODPRODUTO,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      (':QTDE,:UNITARIO,:TOTAL,:IPI,:ICMS,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      (':CFOP,:CODGRADE,:SERIAL,:VALOR_IPI,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      (':CLASSIFICACAO_FISCAL,:CST,:VALOR_ICMS,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      (':ICMS_REDUZIDO,:BASE_CALCULO,:MARGEM_AGREGADA,');
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add
      (':pis_base,:pis_valor,:pis_aliquota,:cofins_base,:cofins_valor,:cofins_aliquota)');
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CODNOTA').asstring :=
      frmnotafiscal_menu.qrnota.fieldbyname('codigo').asstring;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CODPRODUTO').asstring :=
      ed_codigo.text;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('QTDE').asfloat :=
      ed_qtde.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('UNITARIO').asfloat :=
      ed_unitario.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('TOTAL').asfloat :=
      ed_total.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('IPI').asfloat :=
      ed_ipi_aliquota.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('ICMS').asfloat :=
      ed_icms_aliquota.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CFOP').asstring :=
      ed_cfop.text;
    ...
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('SERIAL').asstring :=
      copy(ed_serial.text, 1, 25);
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('VALOR_IPI').asfloat :=
      ed_ipi_valor.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CLASSIFICACAO_FISCAL')
      .asstring := ed_classific.text;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CST').asstring :=
      ed_cst.text;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('VALOR_ICMS').asfloat :=
      ed_icms_valor.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('ICMS_REDUZIDO').asfloat
      := ed_icms_reducao.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('BASE_CALCULO').asfloat
      := ed_icms_base.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('MARGEM_AGREGADA')
      .asfloat := margem_agregada;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('BASE_SUB').asfloat :=
      ed_sub_base.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('ICMS_SUB').asfloat :=
      ed_sub_valor.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('ISENTO').asfloat :=
      ed_isenta.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CODLANCAMENTO').asstring
      := frmprincipal.codifica('000032', 10);
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('DESCONTO').asfloat :=
      ed_desconto.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('SUBTOTAL').asfloat :=
      ed_subtotal.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('PESO_BRUTO').asfloat :=
      ed_peso_bruto.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('PESO_LIQUIDO').asfloat
      := ed_peso_liquido.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('Item').asinteger
      := nf_item;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('codigo').asstring :=
      frmprincipal.codifica('000062', 6);
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('GRADE').asstring :=
      ed_grade.text;
    // FrmNota{ TODO 1 -mizaelrocha2014@outlook.com -cBlob : Resolver }Fiscal.qrnotafiscal_item.Params.Parambyname('COMPLEMENTO'         ).AsBlob := ED_COMPLEMENTO.Lines.Text;

    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('pis_base').asfloat :=
      ed_Pis_Base.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('pis_valor').asfloat :=
      ed_pis_valor.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('pis_aliquota').asfloat
      := ed_pis_aliquota.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('cofins_base').asfloat :=
      ed_cofins_base.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('cofins_valor').asfloat
      := ed_cofins_valor.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('cofins_aliquota')
      .asfloat := ed_cofins_aliquota.Value;
    ...
    begin
      ...
      begin
        ...
        query1.Params.Parambyname('cliente').asstring :=
          frmnotafiscal_menu.qrnota.fieldbyname('cliente').asstring;
        query1.Params.Parambyname('codcliente').asstring :=
          frmnotafiscal_menu.qrnota.fieldbyname('codcliente').asstring;
        query1.Params.Parambyname('DATAVENDA').ASdatetime :=
          frmnotafiscal_menu.qrnota.fieldbyname('data').ASdatetime;
        query1.Params.Parambyname('codvenda').asstring :=
          frmnotafiscal_menu.qrnota.fieldbyname('codigo').asstring;
  ...
  begin
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add(' WHERE CODIGO = ''' +
      pcodigo.caption + '''');
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('QTDE').asfloat :=
      ed_qtde.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('UNITARIO').asfloat :=
  ...
  if FrmNotaFiscal.check_desconto.Checked = false then
    ...
    frmnotafiscal_menu.qrnota.fieldbyname('VALOR_PRODUTOS').asfloat :=
      (frmnotafiscal_menu.qrnota.fieldbyname('VALOR_PRODUTOS').asfloat +
      (ed_total.Value + ed_desconto.Value)) - valor_anterior;
...
begin
  ...
  begin
    application.messagebox('Produdo Sem Preço de Venda!, Verifique', 'Atenção',
...
begin
  ...
  begin
    ...
    begin
      query1.close;
      query1.sql.clear;
      query1.sql.add('select * from c000083 where CODIGO = ''' +
        ed_cst.text + '''');
      query1.open;
      if query1.RecordCount > 0 then
      begin
      ...
      begin
        application.messagebox('CST não cadastrada!', 'Aviso',
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            IF (ed_cst.text = '060') or (ed_cst.text = '010') or
              (ed_cst.text = '070') then
            BEGIN
              margem_agregada := query1.fieldbyname('margem_agregada').asfloat;
              if margem_agregada <> 0 then
              begin
                ed_sub_base.Value := (ed_total.Value * (margem_agregada / 100))
                  * (ed_icms_reducao.Value / 100);
                ed_sub_valor.Value := ed_sub_base.Value *
                  (ed_icms_aliquota.Value / 100);
              end
              else
              begin
                ed_sub_base.Value := 0;
          ...
          begin
            ...
            ed_icms_valor.Value := ed_icms_base.Value *
              (ed_icms_aliquota.Value / 100);
            ed_sub_base.Value := 0;
            ed_sub_valor.Value := 0;
          end;
        ...
        begin
          ...
          // opção para a ouro preto  nota de consignação
          begin
            ed_icms_base.Value := ed_total.Value;
            ed_icms_valor.Value := 0;
          end
...
begin
  ...
  vdesconto_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('DESCONTO').asfloat;
  qtde_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname('qtde').asfloat;
  valor_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('subtotal').asfloat;
  icms_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('valor_icms').asfloat;
  ipi_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('valor_ipi').asfloat;
  base_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('base_calculo').asfloat;
  base_sub_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('base_sub').asfloat;
  icms_sub_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('icms_sub').asfloat;
  PIS_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ('PIS_VALOR').asfloat;
  cofins_anterior := FrmNotaFiscal.qrnotafiscal_item.fieldbyname
    ...
    begin
      ...
      begin
        IF (ed_cst.text = '060') OR (ed_cst.text = '010') or
          (ed_cst.text = '070') THEN
        BEGIN
          if frmmodulo.qrFilial.fieldbyname('SUBSTITUTO_TRIBUTARIO')
            .asstring = 'S' THEN
          begin
            ed_sub_base.Value := (ed_total.Value * (margem_agregada / 100)) *
              (ed_icms_reducao.Value / 100);
      ...
      begin
        ...
        IF (ed_cst.text = '060') OR (ed_cst.text = '010') or
          (ed_cst.text = '070') THEN
        BEGIN
          if frmmodulo.qrFilial.fieldbyname('SUBSTITUTO_TRIBUTARIO').asstring = 'S'
          then
          begin
...
begin
  if ed_cofins_aliquota.Value > 0 then
    ed_cofins_valor.Value := ed_cofins_base.Value *
      (ed_cofins_aliquota.Value / 100);
