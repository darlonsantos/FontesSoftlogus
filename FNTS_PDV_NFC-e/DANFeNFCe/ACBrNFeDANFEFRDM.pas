...
uses
  ...
  DBClient, frxDBSet, pcnConversao, frxBarcode, frxExportText;
...
type
  TdmACBrNFeFR = class(TDataModule)
    ...
    cdsCalculoImpostovTroco: TCurrencyField;
    cdsCalculoImpostovTotPago: TCurrencyField;
  ...
  public
    ...
    property ImgQrCode: string read FImgQrCode write FImgQrCode;
    ...
    begin
      ...
      begin
        if TpAmb = taHomologacao then
          ...
          FieldByName('MensagemFiscal').AsString := DFeUtil.SeSenao(tpEmis = teOffLine, 'EMITIDA EM CONTIGÊNCIA', 'ÁREA DE MENSAGEM FISCAL');
      end else
...
begin
  ...
  begin
    ...
    // Carregamento da imagem
    if DFeUtil.NaoEstaVazio(FImgQrCode) then
    begin
      vStream := TMemoryStream.Create;
      try
        if FileExists(FImgQrCode) then
           vStream.LoadFromFile(FImgQrCode)
        else
        begin
           vStringStream:= TStringStream.Create(FImgQrCode);
           try
              vStream.LoadFromStream(vStringStream);
           finally
              vStringStream.Free;
        ...
        vStream.Position := 0;
        cdsParametrosQrCodeCarregado.LoadFromStream(vStream);
      finally
        vStream.Free;
      end;
    end;
    ...
    begin
      ...
      else // Roberto Godinho - emissão de NFC-e em modo contingência off-line
