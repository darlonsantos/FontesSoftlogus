...
uses
  ...
  AdvToolBar, AdvToolBarStylers, AdvGlowButton, XPMan;
...
uses Leitura_Memoria_Fiscal, Download_MFD, Arquivo_Fiscal,
  ...
  msg_Operador, tabela_indice, Constantes, UFuncoes;

{$R *.dfm}

// -------------------------------------------------------------------------- //
procedure TfrmMenu_Fiscal.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
  ...
  case iECF_Modelo of
    ...
    DARUMA:
      begin
        ...
        begin
          application.messagebox(pwidechar(sMsg), 'Aviso',
...
begin
  ...
  if FileExists(SystemDrive + '\TabProd.txt') then
    DeleteFile(SystemDrive + '\TabProd.txt');
  ...
  writeln(txt, 'P1' + texto_justifica(somenteNumero(sEmpresa_CNPJ), 14, ' ',
    taEsquerda) + texto_justifica(somenteNumero(sEmpresa_IE), 14, ' ',
    taEsquerda) + texto_justifica(somenteNumero(sEmpresa_IM), 14, ' ',
    taEsquerda) + texto_justifica(sEmpresa_Nome, 50, ' ', taEsquerda));
  ...
  begin
    ...
    writeln(txt, 'P2' + texto_justifica(somenteNumero(sEmpresa_CNPJ), 14, '0',
      taEsquerda) + texto_justifica(query.fieldbyname('codigo').asstring, 14,
      ' ', taEsquerda) + texto_justifica(query.fieldbyname('nome').asstring, 50,
      ' ', taEsquerda) + texto_justifica(query.fieldbyname('unidade').asstring,
      6, ' ', taEsquerda) + 'T' + 'T' + texto_justifica(query.fieldbyname('ST')
      .asstring, 1, ' ', taEsquerda) + texto_justifica
      (somenteNumero(formatfloat('##0.00', query.fieldbyname('aliquota')
      .asfloat)), 4, '0', taDireita) + texto_justifica
      (somenteNumero(formatfloat('########0.00',
      query.fieldbyname('preco_venda').asfloat)), 12, '0', taDireita));
    query.next;
    inc(iCont);
  end;

  writeln(txt, 'P9' + texto_justifica(somenteNumero(sEmpresa_CNPJ), 14, ' ',
    taEsquerda) + texto_justifica(somenteNumero(sEmpresa_IE), 14, ' ',
    taEsquerda) + texto_justifica(inttostr(iCont), 6, '0', taDireita));
...
begin
  ...
  rewrite(txt);
  // identificacao da empresa
  writeln(txt, 'E1' + texto_justifica(somenteNumero(sEmpresa_CNPJ), 14, ' ',
    taEsquerda) + texto_justifica(somenteNumero(sEmpresa_IE), 14, ' ',
    taEsquerda) + texto_justifica(somenteNumero(sEmpresa_IM), 14, ' ',
    taEsquerda) + texto_justifica(sEmpresa_Nome, 50, ' ', taEsquerda));
  ...
  begin
    ...
    // Relação de Mercadorias e Serviços
    writeln(txt, 'E2' + texto_justifica(somenteNumero(sEmpresa_CNPJ), 14, '0',
      taEsquerda) + texto_justifica(query.fieldbyname('codigo').asstring, 14,
      ' ', taEsquerda) + texto_justifica(query.fieldbyname('nome').asstring, 50,
      ' ', taEsquerda) + texto_justifica(query.fieldbyname('unidade').asstring,
      6, ' ', taEsquerda) + texto_justifica
      (somenteNumero(formatfloat('########0.00', rEstoque)), 9, '0', taDireita)
      + texto_justifica(somenteNumero(troca_data(datetostr(dData_Sistema))), 8,
      '0', taDireita));
  ...
  writeln(txt, 'E9' + texto_justifica(somenteNumero(sEmpresa_CNPJ), 14, ' ',
    taEsquerda) + texto_justifica(somenteNumero(sEmpresa_IE), 14, ' ',
    taEsquerda) + texto_justifica(inttostr(iCont), 6, '0', taDireita));
...
begin
  frmMsg_operador.lb_msg.Caption :=
    'Aguarde! Imprimindo Identificação do PAF-ECF...';
  frmMsg_operador.Show;
  frmMsg_operador.Refresh;
  ...
  try
    repeat
      ...
      begin
        ...
        if application.messagebox(pwidechar('Erro ao abrir relatório gerencial!'
    ...
    begin
      ...
      begin
        repeat
          case I of
            1:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'a)No.do Laudo......: ' +
                sPAF_Laudo);
            2:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo,
                'b)Identificação da empresa desenvolvedora');
            3:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  b1)CNPJ..........: ' +
                sPAF_CNPJ);
            4:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  b2)Razão Social..: ' +
                sPAF_Empresa);
            5:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  b3)Endereço......: ' +
                sPAF_Endereco);
            6:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  b4)Telefone......: ' +
                sPAF_Telefone);
            7:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  b6)Contato.......: ' +
                sPAF_Contato);
            8:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo,
                'c)Identificação do PAF-ECF');
            9:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  c1)Nome Comercial: ' +
                sPAF_Nome);
            10:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  c2)Versão........: ' +
                sPAF_Versao);
            11:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  c3)Principal EXE.: ' +
                sPAF_Executavel);
            12:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  c4)Cód. MD5 EXE..: ' +
                sPAF_MD5);
            13:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, '  c5)Outros Arq/MD5: ');
            14:
              sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'd)' + sECF_Serial);
          end;
          if sMsg <> OK then
          begin
            frmMsg_operador.Hide;
...
begin
  frmMsg_operador.lb_msg.Caption :=
    'Aguarde! Imprimindo Controle de Encerrantes...';
  frmMsg_operador.Show;
  frmMsg_operador.Refresh;
  ...
  try
    ...
    begin
      repeat
        ...
        query.sql.clear;
        query.sql.add
          ('select posto_bico_movimento.*, posto_bico.*, estoque.nome as produto');
        ...
        begin
          sMsg := cECF_Usa_Gerencial(iECF_Modelo,
            // 'Tanque '+query.fieldbyname('Tanque').asstring+' '+
            'Bomba ' + query.fieldbyname('Bomba').asstring + ' ' + 'Bico ' +
            query.fieldbyname('bico').asstring + ' ' +
            query.fieldbyname('produto').asstring + ', ' + 'EI=' +
            zerar(somenteNumero(formatfloat('######0.00',
            query.fieldbyname('EI').asfloat)), 6) + ', ' + 'EF=' +
            zerar(somenteNumero(formatfloat('######0.00',
            query.fieldbyname('EF').asfloat)), 6) + ', ' + 'Volume=' +
            formatfloat('#########0.000', query.fieldbyname('volume').asfloat));
          query.next;
        end;
        if sMsg <> OK then
        begin
...
begin
  frmMsg_operador.lb_msg.Caption :=
    'Aguarde! Imprimindo Transferência entre Mesas...';
  frmMsg_operador.Show;
  frmMsg_operador.Refresh;
    ...
    begin
      repeat
        ...
        frmmodulo.query_servidor.sql.add
          ('select r000002.*, c000025.produto from r000002, c000025');
        frmmodulo.query_servidor.sql.add
          ('where r000002.cod_produto = c000025.codigo and');
        frmmodulo.query_servidor.sql.add('r000002.transf_mesa > 0');
        frmmodulo.query_servidor.sql.add
          ('order by r000002.cod_mesa, r000002.codigo');
        frmmodulo.query_servidor.open;

        frmmodulo.query_servidor.first;
        while not frmmodulo.query_servidor.eof do
        begin
          sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'Mesa de Origem.: ' +
            zerar(frmmodulo.query_servidor.fieldbyname('transf_mesa')
            .asstring, 3));
          sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'Mesa de Destino: ' +
            zerar(frmmodulo.query_servidor.fieldbyname('cod_mesa')
            .asstring, 3));
          sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'Produto........: ' +
            frmmodulo.query_servidor.fieldbyname('produto').asstring);
          sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'Quantidade.....: ' +
            texto_justifica(formatfloat('###,###,##0.000',
            frmmodulo.query_servidor.fieldbyname('qtde').asfloat), 12, ' ',
            taDireita));
          sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'Unitário.......: ' +
            texto_justifica(formatfloat('###,###,##0.000',
            frmmodulo.query_servidor.fieldbyname('unitario').asfloat), 12, ' ',
            taDireita));
          sMsg := cECF_Usa_Gerencial(iECF_Modelo, 'Total..........: ' +
            texto_justifica(formatfloat('###,###,##0.000',
            frmmodulo.query_servidor.fieldbyname('total').asfloat), 12, ' ',
            taDireita));
          sMsg := cECF_Usa_Gerencial(iECF_Modelo,
            '------------------------------------------------');
    ...
    begin
      repeat
        sMsg := cECF_Usa_Gerencial(iECF_Modelo,
          '------------------------------------------------');
        sMsg := cECF_Usa_Gerencial(iECF_Modelo,
          'No.Mesa   Abertura (Data/Hora)     Total Consumo');
        sMsg := cECF_Usa_Gerencial(iECF_Modelo,
          '------------------------------------------------');
        ...
        frmmodulo.query_servidor.sql.add
          ('r000001.codigo, r000001.data, r000001.hora');
        frmmodulo.query_servidor.sql.add('from r000001, r000002');
        frmmodulo.query_servidor.sql.add
          ('where r000001.codigo = r000002.cod_mesa');
        frmmodulo.query_servidor.sql.add
          ('group by r000001.codigo, r000001.data, r000001.hora');
        ...
        begin
          sMsg := cECF_Usa_Gerencial(iECF_Modelo,
            zerar(frmmodulo.query_servidor.fieldbyname('codigo').asstring, 7) +
            ...
            frmmodulo.query_servidor.fieldbyname('soma').asfloat), 13, ' ',
            taDireita));
          frmmodulo.query_servidor.next;
        end;
...
begin
  frmMsg_operador.lb_msg.Caption :=
    'Aguarde! Imprimindo Abastecimentos Pendentes...';
  frmMsg_operador.Show;
  frmMsg_operador.Refresh;
      ...
      repeat
        ...
        begin
          sMsg := cECF_Usa_Gerencial(iECF_Modelo,
            // 'Tanque '+query.fieldbyname('Tanque').asstring+' '+
            'Bomba ' + query.fieldbyname('Bomba').asstring + ' ' + 'Bico ' +
            query.fieldbyname('bico').asstring + ', ' + 'EI=' +
            zerar(somenteNumero(formatfloat('######0.00',
            query.fieldbyname('EI').asfloat)), 6) + ', ' + 'EF=' +
            zerar(somenteNumero(formatfloat('######0.00',
            query.fieldbyname('EF').asfloat)), 6));
