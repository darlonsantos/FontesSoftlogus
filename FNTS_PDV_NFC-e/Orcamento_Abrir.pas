...
uses
  ...
  DB, MemDS, DBAccess, IBC, Menus, AdvMenus, ComCtrls, XPMan, RxToolEdit;
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          ibcqry1.Close;
          ibcqry1.SQL.Clear;
          ibcqry1.SQL.add('update c000074');
          ibcqry1.sql.add('set dav = '''+sNumero_DAV+''',');

          if grid.cell[10,grid.SelectedRow].asstring <> '' then
          begin
            if sNumero_DAV = grid.Cell[10,grid.SelectedRow].AsString then
              ibcqry1.SQL.add('dav_anterior = '''+
                grid.Cell[1,grid.SelectedRow].AsString+','+
                grid.cell[10,grid.selectedrow].asstring+'''')
            else
            begin
              ibcqry1.SQL.add('dav_anterior = '''+grid.cell[10,grid.selectedrow].asstring+'''');
            end;

          end
          else
          begin
              ibcqry1.SQL.add('dav_anterior = '''+sNumero_Dav+'''')
          end;
          ibcqry1.sql.add('where codigo = '''+grid.cell[0,grid.selectedrow].asstring+'''');
          ibcqry1.ExecSQL;
...
begin
  ...
  ibcqry1.Close;
  ibcqry1.SQL.Clear;
  ibcqry1.SQL.Add('select orc.*, prod.produto, prod.codbarra, prod.cst, prod.comissao, prod.aliquota');
  ibcqry1.sql.add('from c000075 orc,');
  ibcqry1.sql.add('c000025 prod where orc.codproduto = prod.codigo and codnota = '''+
  ...
  if (ibcqry1.RecordCount = 0) and
     (qritem.RecordCount = 0) then
  begin
    Application.messagebox('Não à itens ou serviços lançados para este Orçamento!','Atenção!',mb_yesno+MB_ICONEXCLAMATION);
    exit;
  end
  else
  begin
